...
const createperson = async (req, res) => { // listen to post request on '/'

    const { fullName, phoneNember, cin, age, pic  } = req.body // destructuring the request body to extract the title, load, and reps
    ...
    //add doc to db
    try {
        const Person = await WorkoutModel.create({ title, load, reps }) // create new workout using the destructured data //i did the change from workout to WorkoutModel
        res.status(200).json(Person) // send a json response with the created workout
    } catch (error) {
